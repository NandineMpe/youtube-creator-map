...
def filter_key(active: Filter) -> str:
    ...
    canonical form (Requirement 11.2).
    """
    datasets = "+".join(sorted(active.datasets))
    classes = "+".join(sorted(c.value for c in active.corpus_classes))
    return f"{classes}~{datasets}"
